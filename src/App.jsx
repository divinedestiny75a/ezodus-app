...
// LIVE Firebase configuration from your project
        ...
        <AuthFormWrapper title="Welcome Back!">
            <form className="space-y-6" onSubmit={handleLogin}>
                ...
                <div>
                    ...
                    <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full p-3 mt-1 text-gray-200 bg-gray-900/50 rounded-lg border border-gray-700 focus:border-yellow-400 focus:ring-2 focus:ring-yellow-400 outline-none" />
                </div>
                <div className="relative">
                    ...
                    <input 
                        ...
                        required 
                        className="w-full p-3 mt-1 text-gray-200 bg-gray-900/50 rounded-lg border border-gray-700 focus:border-yellow-400 focus:ring-2 focus:ring-yellow-400 outline-none pr-10" 
                    />
                    <button 
                        type="button" 
                        onClick={() => setShowPassword(!showPassword)}
                        className="absolute inset-y-0 right-0 top-6 flex items-center px-3 text-gray-400 hover:text-yellow-400"
                    >
                        {showPassword ? <EyeOffIcon /> : <EyeIcon />}
                    </button>
                </div>
                <button type="submit" disabled={loading} className="w-full py-3 font-bold text-black bg-yellow-400 rounded-lg hover:bg-yellow-500 transition-all transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed">
                    {loading ? 'Logging in...' : 'Login'}
                </button>
            </form>
            <p className="text-center text-gray-400">
                Don't have an account? <button onClick={() => onNavigate('signup')} className="font-bold text-yellow-400 hover:underline">Sign Up</button>
            </p>
        </AuthFormWrapper>
    );
};
...
const SignUp = ({ onNavigate }) => {
    ...
    return (
        <AuthFormWrapper title="Create Your Account" subtitle="Start your journey with Social Architect.">
            ...
            <p className="text-center text-gray-400">
                Already have an account? <button onClick={() => onNavigate('login')} className="font-bold text-yellow-400 hover:underline">Login</button>
            </p>
        </AuthFormWrapper>
    );
};

const Onboarding = ({ onFinish, user }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('/.netlify/functions/analyzeBrandVoice', {
                method: 'POST',
                headers: {
            ...
            if (response.ok && result.success) {
                setAnalysisResult(result.brandVoice);
            } else {
                throw new Error(result.error || "Failed to analyze the URL.");
            }
        } catch (err) {
            console.error("Netlify Function Error:", err);
            setError("Sorry, we couldn't analyze the URL. Please ensure it's a valid, public page and try again.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleSaveAndContinue = async () => {
        ...
        await setDoc(userDocRef, { brandVoice: analysisResult }, { merge: true });
        onFinish();
    };
    ...
    if (analysisResult) {
        return (
            <div className="flex items-center justify-center min-h-screen">
                <div className="w-full max-w-lg p-8 space-y-6 bg-black/40 backdrop-blur-lg rounded-xl shadow-2xl shadow-yellow-500/10 border border-yellow-500/30 text-center">
                    <h1 className="text-3xl font-bold text-white">Here's Your Brand Voice</h1>
                    <div className="text-left bg-gray-900/50 p-4 rounded-lg border border-gray-700">
                        {analysisResult.split('*').map((item, index) => item.trim() && <p key={index} className="my-2 text-gray-300">- {item.trim()}</p>)}
                    </div>
                    <p className="text-gray-400">We'll use this profile to generate all your future content. You can refine this later in settings.</p>
                    <button onClick={handleSaveAndContinue} className="w-full py-3 font-bold text-black bg-yellow-400 rounded-lg hover:bg-yellow-500 transition-all transform hover:scale-105">
    ...
    return (
        <div className="flex items-center justify-center min-h-screen">
            <div className="w-full max-w-lg p-8 space-y-6 bg-black/40 backdrop-blur-lg rounded-xl shadow-2xl shadow-yellow-500/10 border border-yellow-500/30 text-center">
                <div className="w-16 h-16 mx-auto bg-yellow-400/10 rounded-full flex items-center justify-center border border-yellow-400/20"><BrandVoiceIcon /></div>
                <h1 className="text-3xl font-bold text-white">Let's Discover Your Brand Voice</h1>
                <p className="text-gray-300">Enter your website URL or a link to your primary social media profile (like Instagram, Facebook, or YouTube).</p>
                <form className="space-y-4" onSubmit={handleSubmit}>
                    ...
                    <div>
                        <input 
                            ...
                            required 
                            className="w-full p-3 text-center text-gray-200 bg-gray-900/50 rounded-lg border border-gray-700 focus:border-yellow-400 focus:ring-2 focus:ring-yellow-400 outline-none" 
                        />
                    </div>
                    <button type="submit" disabled={isLoading} className="w-full py-3 font-bold text-black bg-yellow-400 rounded-lg hover:bg-yellow-500 transition-all transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed">
                        {isLoading ? 'Analyzing...' : 'Analyze Brand Voice'}
                    </button>
                </form>
                <button onClick={onFinish} className="text-sm text-gray-500 hover:text-gray-300 mt-4">Skip for now</button>
            </div>
        </div>
    );
};
...
const SettingsView = ({ user }) => {
    return (
        <div>
            ...
            {/* Account Settings */}
            <div className="bg-black/30 backdrop-blur-md p-8 rounded-xl border border-yellow-500/30 mb-8">
                <h2 className="text-2xl font-bold text-yellow-400 mb-4">Account Information</h2>
                <div className="space-y-4">
                    <div>
                     ...
                     <div>
                        <label className="text-sm font-bold text-gray-400 block">Password</label>
                        <button className="text-yellow-400 hover:underline">Change Password</button>
                    </div>
                </div>
            </div>

            {/* Connected Accounts */}
            <div className="bg-black/30 backdrop-blur-md p-8 rounded-xl border border-yellow-500/30">
                <h2 className="text-2xl font-bold text-yellow-400 mb-4">Connected Accounts</h2>
                <p className="text-gray-400 mb-6">Connect your social media accounts to allow EZODUS to post on your behalf.</p>
                <div className="space-y-4">
                    <div className="flex items-center justify-between p-4 bg-gray-900/50 rounded-lg">
                        <div className="flex items-center">
                           <MetaIcon />
                           <span className="font-bold text-white">Meta (Facebook & Instagram)</span>
                        </div>
...
const VerifyEmail = ({ user, onLogout }) => {
    ...
    const handleResend = async () => {
        try {
            ...
            setMessage('Failed to send verification email. Please try again shortly.');
        ...
        <div className="flex items-center justify-center min-h-screen">
            <div className="w-full max-w-lg p-8 space-y-6 bg-black/40 backdrop-blur-lg rounded-xl shadow-2xl shadow-yellow-500/10 border border-yellow-500/30 text-center">
                ...
                <p className="text-gray-300">
                    We've sent a verification link to <strong>{user.email}</strong>. Please click the link in the email to activate your account.
                </p>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
                    <button onClick={handleResend} className="w-full sm:w-auto py-2 px-6 font-bold text-black bg-yellow-400 rounded-lg hover:bg-yellow-500 transition-all">
                        Resend Email
                    </button>
                    <button onClick={onLogout} className="w-full sm:w-auto text-sm text-gray-400 hover:text-white">
                        Logout
                    </button>
                </div>
            </div>
...
const Dashboard = ({ onLogout, user }) => { 
    ...
    useEffect(() => { 
        ...
        const unsubscribe = onSnapshot(userDocRef, (docSnap) => {
            if (docSnap.exists() && docSnap.data().brandVoice) {
                setBrandVoice(docSnap.data().brandVoice);
            }
        }, (error) => {
    ...
    const renderView = () => { 
        switch (view) { 
            case 'dashboard': return ( <div> <h1 className="text-3xl font-bold mb-2 text-white">Welcome, {user.displayName || user.email}!</h1> <p className="text-gray-400 mb-8">This is your social media command center.</p> <div className="bg-black/30 backdrop-blur-md p-8 rounded-xl border border-dashed border-yellow-500/30 text-center"> <h2 className="text-xl font-bold mb-2 text-yellow-400">Dashboard Overview Coming Soon</h2> <p className="text-gray-300">Key metrics and analytics will be displayed here.</p> </div> </div> ); 
            case 'createPost': return <CreatePostView brandVoice={brandVoice} />; 
            case 'settings': return <SettingsView user={user} />;
            default: return <div> <h2 className="text-xl font-bold mb-2 text-yellow-400">{view} Coming Soon</h2> </div>; 
        } 
    }; 
    return ( 
        <div className="flex h-screen bg-black/50"> 
            <aside className="w-64 bg-black/30 backdrop-blur-md border-r border-yellow-500/20 p-6 hidden md:flex flex-col"> 
                <div className="text-2xl font-bold tracking-wider mb-10 text-white"><span className="text-yellow-400">EZO</span>DUS</div> 
                <nav className="space-y-4"> 
                    <DashboardNavItem icon="🏠" text="Dashboard" active={view === 'dashboard'} onClick={() => setView('dashboard')} /> 
                    <DashboardNavItem icon="✍️" text="Create Post" active={view === 'createPost'} onClick={() => setView('createPost')} /> 
                    <DashboardNavItem icon="📅" text="Calendar" active={view === 'calendar'} onClick={() => setView('calendar')} /> 
                    <DashboardNavItem icon="📈" text="Analytics" active={view === 'analytics'} onClick={() => setView('analytics')} /> 
                    <DashboardNavItem icon="⚙️" text="Settings" active={view === 'settings'} onClick={() => setView('settings')} /> 
                </nav> 
                <div className="mt-auto"><button onClick={onLogout} className="flex items-center space-x-2 text-gray-400 hover:text-white w-full p-3 rounded-lg hover:bg-yellow-500/10"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"></path></svg><span>Logout</span></button></div> 
            </aside> 
    ...
    const renderPage = () => {
        ...
        if (user) {
            ...
            if (!onboardingComplete) {
                return <Onboarding onFinish={handleOnboardingFinish} user={user} />;
            }
